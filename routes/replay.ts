...
router.post('/webhooks/:id/replay', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (method !== 'GET' && method !== 'HEAD') {
            fetchOptions.body = webhook.RawBody ? webhook.rawBody : (webhook.body ? JSON.stringify(webhook.body) : undefined);
        }
        ...
        res.json({
            ...
            replay: {
                ...
                response: responseBody.substring(0, 1000),// Prevent massive payloads from crashing the UI
            }
        });
